...
The structure remains intentionally small. The maintainer has reviewed the Pi, OpenHands SDK, and OpenClaw cases and approved D-011's pinned Hermes Agent case next, with a written scope boundary required before deep analysis. Browser Use remains a deferred perception-grounded contrast for after the first learning-oriented synthesis. Source-native topics may later add navigation without forcing the responsibility lens into an exclusive directory tree.
