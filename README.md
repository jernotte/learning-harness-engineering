...
The structure remains intentionally small. The maintainer has reviewed the Pi, OpenHands SDK, and OpenClaw cases and authorized only pinned Browser Use next, with a written scope boundary required before deep analysis. Source-native topics may later add navigation without forcing the responsibility lens into an exclusive directory tree.
