...
D-011 amended D-010's immediate order and trigger. Hermes Agent `v2026.7.7.2` at `9de9c25f620ff7f1ce0fd5457d596052d5159596` was the final pre-Checkpoint-3 case and began with a written scope boundary. D-013 then approved Checkpoint 3's stable-ID responsibility refinements, cross-cutting annotations, and static-first, question-triggered focused-test policy. D-014 rejected the first broad learning pilot and authorized a mechanism-level context-management replacement. No Claude Code case, Browser Use case, replacement harness, or cohort member is inserted before the maintainer dispositions that active replacement. Browser Use remains deferred so its perception-grounded R5/R6 value stays explicit.

## Cycle 1 consolidation and refinement

Build the first cross-harness comparison around architectural decisions rather than product profiles. Reconcile vocabulary, trace apparent agreement back to independent evidence, and investigate important contradictions. Turn recurring observations into provisional patterns without promoting frequency into effectiveness. After the reviewed Hermes case and approved Checkpoint 3, derive a human learning view from that evidence. Because the first broad pilot failed pedagogically, the active D-014 context-management replacement—not mere file completion—is now the gate before Browser Use or another case may reopen against explicit gaps.
...
**Checkpoint 4:** review whether the synthesis has been converted into a genuinely useful learning artifact, not merely whether it is accurate. The first broad pilot was rejected under D-014; the active replacement is a mechanism-level context-management chapter whose acceptance depends on maintainer learning value as well as research fidelity.
