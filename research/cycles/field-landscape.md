...
Sixteen narrative records cited or used in the synthesis now have referenced dispositions, canonical claim mappings, and primary verification. Browser Use, smolagents, Hermes, Letta, and other Cycle 1 implementation details entered as read-only leads without the durable source lifecycle required to support claims. Checkpoint 2 separately admitted Pi, OpenHands, and OpenClaw through pinned implementation cases. D-011 now admits Hermes identity, exact pin, bounded relationship evidence, and dated activity/attention indicators at selection depth; no reported Hermes mechanism was inherited from the reconstructed report.
...
The original set is documented in `research/selections/checkpoint-2-deep-dive-set.md`: Pi, OpenHands Software Agent SDK, LangGraph, Browser Use, OpenClaw, and a conditional maintainer-supplied Claude Code case. D-009 later deferred LangGraph from the concrete-harness sequence. D-011's bounded amendment selects Hermes at `v2026.7.7.2` / `9de9c25f620ff7f1ce0fd5457d596052d5159596`, defers Browser Use until after the first learning-oriented synthesis, and removes every other pre-Checkpoint-3 branch. The responsibility matrices remain expected-coverage hypotheses, not mechanism claims.
...
The subsequent bounded Hermes amendment verified canonical repository `NousResearch/hermes-agent`, annotated tag `v2026.7.7.2`, dereferenced commit `9de9c25f620ff7f1ce0fd5457d596052d5159596`, and package version `0.18.2`. It classifies the OpenClaw relationship as separate Git roots followed by migration compatibility and selective documented influence, not direct whole-repository descent or independent convergence. Its adoption assessment is limited to dated repository attention, organization ownership, activity, releases, and integrations; no user, deployment, team-size, quality, or effectiveness claim is admitted. No Hermes deep analysis began.

### Checkpoint 3 trigger and agenda

Hermes is the final pre-Checkpoint-3 case. It must begin with a written scope boundary and remain a standalone Hermes analysis, using OpenClaw only for explicit evidence-bounded comparison questions. Checkpoint 3 convenes immediately after the maintainer reviews Hermes. No Browser Use, Claude Code, replacement, cohort member, or alternate may be inserted first.
...
| Checkpoint 3 | First concrete case-study batch | Immediately after maintainer-reviewed Hermes | Not reached |  | Compare accumulated friction and decide focused target-test execution policy | No other implementation before checkpoint |
