...
**Current phase:** Replacement context-management learning experience prepared for content review; promotion audit and Fable review pending
...
The first-batch synthesis has a separate complete promotion-profile audit at `research/provenance/first-batch-synthesis-audit.md`. It opens 15 records, references 14, excludes one infrastructure record, declares 17 claims, and verifies all 65 exact mappings. The bounded window contains 95 native observations, 94 automatic classifications, and one local-repository exception resolved in one batch, with one verified archive, no search, and no manually authored result event. Two selection-depth warnings remain visible and are resolved only for their explicit gap-claim role. The maintainer later rejected its first human derivative as too abstract and shallow; that rejection does not invalidate the research synthesis.

D-014 authorizes a replacement mechanism-level context package. It reuses the four reviewed cases, adds bounded pinned Codex CLI and Letta topic inspections without promoting either to a full case, and uses Claude Code only as secondary reporting. The working synthesis and claim ledger are `research/syntheses/context-management-across-harnesses.md` and `research/claims/context-management-across-harnesses.md`; the canonical human layer is `learning/chapters/context-management-in-agent-harnesses.md`. The four reused source records now reciprocally name the new synthesis mappings. The package remains a content-review draft because its dedicated canonical event log and generated source/claim audit are still pending. Independent Fable review remains deferred by capacity rather than passed.
...
The maintainer approved D-013 at Gate B. The four-case comparison retains stable R1–R11 identifiers, sharpens R9 and R11, adds bounded cross-cutting annotations, and adopts static-first analysis with focused pinned test execution for predeclared load-bearing ambiguities. The maintainer rejected the first derived learning pilot and approved D-014's context-management replacement. That replacement Markdown and derived static site are prepared for maintainer reading; their deferred Fable review has not run.
...
| Checkpoint 4 — rejected pilot | First-batch synthesis, claim ledger, “Where Harnesses Put Control” chapter, and generated site | 2026-07-19 | Rejected as a learning artifact | Maintainer accepted presentation but rejected the chapter as abstract and shallow | Preserve the research substrate; replace the human curriculum rather than polishing the rejected page | D-014 context-management replacement only |
| Checkpoint 4 — replacement | Context-management synthesis, 15-claim working ledger, human chapter, and regenerated site | 2026-07-19 | Prepared for content review; not promotion-ready | Pending | Dedicated provenance/source/claim audit and Fable review remain pending; Codex/Letta are bounded topic inspections and Claude is secondary | None |
