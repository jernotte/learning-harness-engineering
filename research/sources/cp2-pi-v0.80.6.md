...
| CP2-S001-E12 | The coding read tool keeps a bounded head with continuation, while bash keeps a bounded tail and persists the full truncated output; both default to 2,000 lines or 50 KiB. Compaction is enabled by default with 16,384 reserved tokens and a 20,000-token recent region. | `packages/coding-agent/src/core/tools/truncate.ts` 1–12, 71–150, 164–231; `read.ts` 203–214, 264–310; `bash.ts` 291–302, 370–390; `packages/coding-agent/docs/compaction.md` 25–45, 79–85, 376–396 | `field-landscape-synthesis-context-management-across-harnesses-C004`, `field-landscape-synthesis-context-management-across-harnesses-C005` | supports | Exact pinned defaults and mechanics; semantic retention and task-outcome effects remain unmeasured |
...
The working context-management synthesis reuses CP2-S001-E04–E08 and E12 for `field-landscape-synthesis-context-management-across-harnesses-C001` through `field-landscape-synthesis-context-management-across-harnesses-C006` where the canonical ledger names Pi, and CP2-S001-E05–E07 for `field-landscape-synthesis-context-management-across-harnesses-C008`. The reviewed case boundary also contextualizes `field-landscape-synthesis-context-management-across-harnesses-C012`. These are new synthesis mappings over already reviewed evidence; they do not make Pi's compaction, truncation, or child-context choices outcome-supported.
